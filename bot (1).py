...
def _extract_sudrf_rows(soup: BeautifulSoup) -> list[dict]:
    ...
    КЛЮЧЕВОЕ ПРАВИЛО: строка с реальным делом ВСЕГДА содержит ссылку
    с параметром name_op=case. Строки без неё — заголовки и разделители —
    полностью игнорируем. Это исключает ложные срабатывания.
    """
    cases = []

    # Ищем таблицу: сначала div#tablcont → table, потом fallback варианты
    div = soup.find("div", id="tablcont")
    ...
    if not table:
        for t in soup.find_all("table"):
            if t.find("a", href=re.compile(r"name_op=case")):
                table = t
                break

    if not table:
        log.debug("Таблица с делами не найдена — дел нет")
        return cases

    for row in table.find_all("tr"):
        # Только строки со ссылкой на конкретное дело
        case_link = row.find("a", href=re.compile(r"name_op=case"))
        if not case_link:
            continue

        number = case_link.get_text(strip=True)
        ...
        cases.append({
            "number":        number,
            "hearing_date":  safe(1),
            "case_category": safe(2),
            "parties":       safe(3),
            "judge":         safe(4),
            "status":        safe(5),
        })

    log.info("_extract_sudrf_rows: найдено %d дел", len(cases))
    return cases
